...
impl Thread {
    ...
    ///
    /// Maps to: `src/agent/session.rs` → `Thread::truncate_turns()`
    ///
    /// ## What's new (compared to mini-agent-memory):
    ///
    /// This method enables context compaction — when the conversation grows
    /// too long, old turns can be removed to keep the context within limits.
    /// In IronClaw, this is called by `ContextCompactor` when the
    /// `ContextMonitor` detects the context is approaching the limit.
    pub fn truncate_turns(&mut self, keep: usize) {
        if self.turns.len() > keep {
            ...
            info!(
                ...
                keep = keep,
                removing = drain_count,
                "✂️  Truncating thread: keeping {} most recent turns, removing {}",
                keep, drain_count
            );
