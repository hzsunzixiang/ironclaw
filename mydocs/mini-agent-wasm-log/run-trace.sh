#!/usr/bin/env bash
#
# run-trace.sh — Run mini-agent-wasm with RUST_LOG=trace and save logs
#
# Usage:
...
echo "╔════════════════════════════════════════════════╗"
echo "║   Mini Agent Loop + WASM — Trace Runner        ║"
echo "╠════════════════════════════════════════════════╣"
...
# Only trace OUR code (mini_agent_wasm); suppress noisy third-party crates
# wasmtime internals (type_registry, cranelift, etc.) produce thousands of
# unreadable TRACE lines — keep them at warn level.
RUST_LOG_FILTER="mini_agent_wasm=trace,wasmtime=warn,cranelift=warn,regalloc=warn,wasi=warn"
