...
//! # Agentic Loop — The Core Engine (with Turn + Memory integration)
//!
...
//! |-------------------------------|------------------------------------------|
//! | No persistent memory           | MEMORY.md injected into system prompt    |
//! | No memory tools                | memory_search, memory_write, memory_read |
//! | No auto-extraction             | Auto-extract memories from turns          |
//!
//! Key additions:
//!   - System prompt includes MEMORY.md content (long-term memory injection)
//!   - After each turn, auto-extract memories from conversation
//!   - Memory tools allow LLM to explicitly search/write memories
