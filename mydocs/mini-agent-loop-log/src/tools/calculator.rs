...
impl Tool for CalculatorTool {
    ...
    async fn execute(&self, params: serde_json::Value) -> Result<ToolOutput, String> {
        ...
        debug!(params = %serde_json::to_string_pretty(&params).unwrap_or_default(), "Calculator input params");
        ...
        debug!(operation = %op, a = a, b = b, "Parsed: {} {} {}", a, op, b);
        ...
        let (result, expression) = match op {
            ...
            "div" => {
                if b == 0.0 {
                    tracing::error!(a = a, b = b, "Division by zero attempted");
                    return Err("Division by zero".to_string());
                }
                (a / b, format!("{} ÷ {} = {}", a, b, a / b))
            }
            _ => {
                tracing::error!(operation = %op, "Unknown operation");
                return Err(format!("Unknown operation: '{}'", op));
            }
        };

        info!(expression = %expression, result = result, "✅ Calculator result: {}", expression);
        trace!(
            output = %serde_json::to_string(&serde_json::json!({"result": result, "expression": &expression})).unwrap_or_default(),
